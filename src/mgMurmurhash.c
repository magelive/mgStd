...
static inline uint32_t mg_rotl32(uint32_t x, int8_t r )
{
      return (x << r) | (x >> (32 - r));
}

static inline uint64_t mg_rotl64 ( uint64_t x, int8_t r )
{
      return (x << r) | (x >> (64 - r));
}

#define ROTL32(x,y) mg_rotl32(x,y)
#define ROTL64(x,y) mg_rotl64(x,y)
