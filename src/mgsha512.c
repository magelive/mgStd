...
{
    ctx->H[0] = 0x6a09e667f3bcc908; ctx->H[1] = 0xbb67ae8584caa73b;
    ctx->H[2] = 0x3c6ef372fe94f82b; ctx->H[3] = 0xa54ff53a5f1d36f1;  
    ctx->H[4] = 0x510e527fade682d1; ctx->H[5] = 0x9b05688c2b3e6c1f;
    ctx->H[6] = 0x1f83d9abfb41bd6b; ctx->H[7] = 0x5be0cd19137e2179;
    ...
    ctx->K[16] = 0xe49b69c19ef14ad2; ctx->K[17] = 0xefbe4786384f25e3;//0xefbe4786512f25e3;
...
static unsigned long long mg_sha512_shift_num3(unsigned long long input, int OpOne, int OpTwo, int Opthree) 
{ 
...
static void mg_sha512_setw(MG_SHA512_CTX *ctx) 
{ 
    int index = 0; 
    ...
    for(index = 16; index < SHA512_ROUNDNUM; index++) 
    { 
        s0 = 0; 
        s1 = 0; 
        s0 = mg_sha512_shift_num3(ctx->W[index-2],19,61,-6); 
        s1 = mg_sha512_shift_num3(ctx->W[index-15],1,8,-7); 
        ctx->W[index] = (s0 + ctx->W[index-7] + s1 + ctx->W[index-16]); 
    } 
} 

static void mg_sha512_round(MG_SHA512_CTX *ctx) 
{ 
    ...
    for(index = 0;index < SHA512_ROUNDNUM ;index++) 
    { 
        s0 = 0; 
        s1 = 0; 
        s0 = mg_sha512_shift_num3(A, 28, 34, 39); 
        t2 = s0 + ((A&B)^(A&C)^(B&C)); 
        s1 = mg_sha512_shift_num3(E, 14, 18, 41); 
...
void mg_sha512_final(unsigned char output[SHA512_DIGEST_LEN], MG_SHA512_CTX *ctx)
{
    ...
    int i;
    ctx->LastString[realLen%(SHA512_BLOCKWIDTHNUM*8)] = 0x80;
    if(ctx->LastStringLen <= SHA512_BLOCKWIDTHNUM*8-1-16)  
    {
        flag = 1;
        for( i = 8; i >= 1; i--) 
            ctx->LastString[SHA512_BLOCKWIDTHNUM*8 - i] = (char)((realLen<<3)>>((i-1)*8));   
    } 
    else 
    { 
        flag = 2;
        for( i = 1; i <= 8; i++) 
            ctx->LastString[SHA512_BLOCKWIDTHNUM*8*2 - i] = (char)((realLen<<3)>>((i-1)*8));   
    }
    while(LastRoundCount <= flag)
    {
        memcpy(ctx->M, (ctx->LastString+SHA512_BLOCKWIDTHNUM*8*(LastRoundCount - 1)),
                SHA512_BLOCKWIDTHNUM*8);//最后的一段或两段 
        LastRoundCount++;
        mg_sha512_setw(ctx);
        mg_sha512_round(ctx);
    }
    for(i = 0; i < SHA512_CONSTNUM; i++)
    {
...
{
    MG_SHA512_CTX mg_sha512_ctx;
    mg_sha512_init(&mg_sha512_ctx);
    mg_sha512_update(&mg_sha512_ctx, buf, buf_len);
    mg_sha512_final(output, &mg_sha512_ctx);
}
...
void mg_sha512_file(unsigned char output[SHA512_DIGEST_LEN], char *filename)
{
    ...
    {
        mg_sha512_update(&mg_sha512_ctx, buf, strlen(buf));
        bzero(buf, 1024);
    }
    mg_sha512_final(output, &mg_sha512_ctx);
    fclose(fp);
